...
export const Dashboard: React.FC<ExtendedDashboardProps> = ({ 
  ...
  // Hover states now allow holding both metrics
  ...
  const loadWeatherData = useCallback(async (lat: number, lon: number, cityName?: string) => {
    setLoading(true);
    // Update coordinates for refresh capability
    ...
    try {
        ...
        // Reverse geocode if name is missing (e.g. Earth click)
        if (!finalName) {
            try {
  ...
  useEffect(() => {
      const handleEarthClickEvent = (e: CustomEvent) => {
          ...
          setLocationAccuracy(null); // Reset accuracy on manual map selection
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      // If no suggestion selected, allow default form submit which calls handleSearch
      ...
      if ('geolocation' in navigator) {
          navigator.geolocation.getCurrentPosition(
              async (position) => {
                  ...
                  setLocationAccuracy(Math.round(accuracy)); // Store accuracy
  ...
  return (
    <div className="relative w-full h-screen text-white overflow-hidden font-sans selection:bg-[#00C2FF] selection:text-black pointer-events-none">
      ...
      <div className="absolute top-24 right-4 z-20 flex flex-col space-y-2 pointer-events-auto">
        <button 
            onClick={onZoomIn}
            className="p-2 bg-[#0B0E14]/80 backdrop-blur-md border border-white/10 rounded-lg hover:bg-white/10 text-gray-300"
            title="Zoom In"
        >
        ...
        <button 
            ...
            className="p-2 bg-[#0B0E14]/80 backdrop-blur-md border border-white/10 rounded-lg hover:bg-white/10 text-[#00C2FF] mt-2"
           ...
           {loading ? (
              ...
              <>
                ...
                <button className={`w-full flex flex-col px-4 py-3 text-sm transition-all border-l-2 border-transparent text-gray-400 hover:bg-white/5`}>
                   ...
                   <div className="w-full h-1 bg-gray-700 rounded-full overflow-hidden">
                       <div 
                           className="h-full bg-gradient-to-r from-green-400 via-yellow-400 to-purple-600" 
                    ...
                    <div className="relative w-full h-full group">
                        ...
                        <div className="absolute inset-0 w-full h-full flex items-stretch">
                           {points.map((p, i) => (
                               ...
                               >
                                   <div 
                                       className="absolute w-1.5 h-1.5 bg-white rounded-full opacity-0 group-hover/point:opacity-100 shadow-[0_0_8px_#00C2FF] pointer-events-none"
                                       style={{ 
                                           top: `${p.y}%`, 
                                           left: '50%', 
                        ...
                        {hoveredPoint && (
                            <div 
                               className="absolute bg-[#0B0E14]/95 backdrop-blur-md border border-white/20 rounded-lg p-2 pointer-events-none z-50 shadow-2xl min-w-[100px]"
                               style={{ 
                                   left: `${hoveredPoint.x}%`, 
                                   top: `${hoveredPoint.y}%`, 
                    ...
                    <div className="relative w-full h-full group">
                        ...
                        <div className="absolute inset-0 w-full h-full flex items-stretch">
                           {points.map((p, i) => (
                               ...
                               >
                                   <div 
                                       className="absolute w-1.5 h-1.5 bg-white rounded-full opacity-0 group-hover/point:opacity-100 shadow-[0_0_8px_#2dd4bf] pointer-events-none"
                                       style={{ 
                                           top: `${p.y}%`, 
                                           left: '50%', 
                                           transform: 'translate(-50%, -50%)' 
                      ...
                      return (
                          <div key={i} className="flex flex-col items-center flex-1 group">
                              <div className="relative w-full bg-[#161B26] rounded-t-sm overflow-hidden h-16 flex items-end">
                                  <div 
                                    className="w-full bg-blue-500/80 transition-all duration-500 group-hover:bg-blue-400"
                                    style={{ height: `${prob}%` }}
                                  ></div>
                              </div>
            ...
            <div className="flex flex-col space-y-2">
               {recentLocations.map((loc, idx) => (
                 <button 
                   ...
                   className="flex items-center justify-between w-full p-2 rounded-lg bg-[#161B26] hover:bg-white/5 transition-colors group"
                 >
                    <div className="flex items-center">
