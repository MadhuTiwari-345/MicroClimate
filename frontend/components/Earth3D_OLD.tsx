...
export const Earth3D: React.FC<Earth3DProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Create a rotation wrapper group for all earth components
    const rotationGroup = new THREE.Group();
    earthGroup.add(rotationGroup);
    rotationGroupRef.current = rotationGroup;

    // Textures - load directly without callbacks for simplicity
    const textureLoader = new THREE.TextureLoader();
    const earthTexture = textureLoader.load('https://unpkg.com/three-globe/example/img/earth-blue-marble.jpg');
    
    // Earth Geometry - match TutorialEarth
    const geometry = new THREE.SphereGeometry(1, 64, 64);
    const material = new THREE.MeshPhongMaterial({
      map: earthTexture,
      specular: new THREE.Color(0x333333),
      shininess: 15,
      color: 0x1a5fa0, // Ocean blue fallback
    });
    ...
    // Atmosphere Glow - match TutorialEarth with MeshBasicMaterial
    ...
    // Lighting - simplified to match TutorialEarth
    ...
    let frameId = 0;
        const animate = () => {
            ...
            try {
                if (rotationGroupRef.current && !isDragging) {
                    // Smoothly interpolate towards target rotation (e.g. when clicking a saved view)
                    // Or auto-rotate if no specific target
                    if (!showMarker) {
                        // Simple direct rotation like TutorialEarth for smooth animation
                        rotationGroupRef.current.rotation.y += 0.002;
                        targetRotationRef.current.y = rotationGroupRef.current.rotation.y;
                    } else {
                        // Damping effect for smooth transition to location
                        rotationGroupRef.current.rotation.y += (targetRotationRef.current.y - rotationGroupRef.current.rotation.y) * 0.05;
                        rotationGroupRef.current.rotation.x += (targetRotationRef.current.x - rotationGroupRef.current.rotation.x) * 0.05;
                    }
                }
