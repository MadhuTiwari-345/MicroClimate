...
export const Profile: React.FC<ProfileProps> = ({ onNavigate, onLogout, user, onLocationSelect }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0E14] text-white font-sans selection:bg-[#00C2FF] selection:text-black">
      {/* Top Navigation Bar */}
      <nav className="border-b border-white/5 bg-[#0B0E14] sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center cursor-pointer group" onClick={() => onNavigate('landing')}>
              <div className="flex items-center justify-center w-8 h-8 mr-3 bg-[#00C2FF]/10 rounded-lg group-hover:bg-[#00C2FF]/20 transition-colors">
                <Diamond className="w-5 h-5 text-[#00C2FF] transform rotate-45" fill="currentColor" />
              </div>
              <span className="text-lg font-bold tracking-tight text-white">MicroClimate</span>
            </div>

            <div className="hidden md:flex items-center space-x-8">
              <button onClick={() => onNavigate('dashboard')} className="text-sm font-medium text-gray-400 hover:text-white transition-colors">Dashboard</button>
              <button onClick={() => onNavigate('explore')} className="text-sm font-medium text-gray-400 hover:text-white transition-colors">Map Explorer</button>
              <button onClick={() => onNavigate('alerts')} className="text-sm font-medium text-gray-400 hover:text-white transition-colors">Data Insights</button>
              <button className="text-sm font-medium text-[#00C2FF] relative py-5 border-b-2 border-[#00C2FF]">Profile</button>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span className="absolute top-2 right-2 w-2 h-2 bg-[#00C2FF] rounded-full border-2 border-[#0B0E14]"></span>
              </button>
              <div className="w-8 h-8 rounded-full bg-[#1E2330] border border-white/10 flex items-center justify-center overflow-hidden">
                 <div className="w-full h-full bg-gradient-to-br from-[#FFE4C4] to-[#FFDAB9] flex items-end justify-center">
                    <User className="w-6 h-6 text-gray-600 translate-y-1" /> 
                 </div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <button onClick={() => onNavigate('dashboard')} className="flex items-center text-gray-400 hover:text-[#00C2FF] mb-6 transition-colors group">
            <ArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" />
            Back to Dashboard
        </button>

        <div className="relative mb-8 p-6 rounded-2xl bg-[#161B26] border border-white/5 overflow-hidden">
          ...
          <div className="flex items-center space-x-6 relative z-10">
            <div className="relative">
              <div className="w-20 h-20 rounded-full bg-[#1E2330] border-2 border-[#00C2FF] flex items-center justify-center overflow-hidden shadow-[0_0_20px_rgba(0,194,255,0.3)]">
                 <div className="w-full h-full bg-gradient-to-br from-[#FFE4C4] to-[#FFDAB9] flex items-end justify-center">
                    <User className="w-16 h-16 text-gray-600 translate-y-2" /> 
                 </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1 space-y-1">
            {menuItems.map((item) => (
              ...
              >
                <item.icon className={`w-4 h-4 mr-3 ${activeTab === item.id ? 'text-[#00C2FF]' : 'text-gray-500'}`} />
                {item.name}
              </button>
            ))}
            ...
            <div className="pt-4 mt-4 border-t border-white/5 space-y-1">
              <button 
                ...
                className="w-full flex items-center px-4 py-3 text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition-all"
              >
                <LayoutTemplate className="w-4 h-4 mr-3 text-gray-500" />
                Admin Panel
              </button>
              <button onClick={handleLogout} className="w-full flex items-center px-4 py-3 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-all">
          ...
          <div className="lg:col-span-3 bg-[#0B0E14] rounded-2xl border border-white/5 p-6 min-h-[500px]">
            {activeTab === 'settings' && (
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-gray-300">Temperature Units</label>
                      <div className="relative">
                        <select 
                          value={tempUnit}
                          onChange={(e) => setTempUnit(e.target.value)}
                          className="w-full appearance-none bg-[#161B26] border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-[#00C2FF] transition-colors"
                        >
                    ...
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-gray-300">Wind Speed Units</label>
                      <div className="relative">
                        <select 
                          value={windUnit}
            ...
            {activeTab === 'account' && (
              <div className="animate-fade-in">
                 ...
                 <div className="space-y-6 max-w-xl mt-6">
                   <input type="text" value={name} onChange={e=>setName(e.target.value)} className="w-full bg-[#161B26] border border-white/10 rounded-lg px-4 py-2.5 text-white" />
                   <input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-[#161B26] border border-white/10 rounded-lg px-4 py-2.5 text-white" />
                 </div>
              </div>
            )}
                 ...
                 {savedLocations.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {savedLocations.map((loc, i) => (
                            <div key={i} onClick={() => handleLocationClick(loc)} className="bg-[#161B26] border border-white/5 p-4 rounded-xl flex items-center justify-between cursor-pointer hover:border-[#00C2FF]/50 transition-all group">
                                ...
                                <ArrowLeft className="w-4 h-4 text-gray-500 rotate-180 group-hover:translate-x-1 transition-transform" />
                            </div>
                        ))}
                    </div>
                 ) : (
            ...
            {activeTab === 'security' && (
               <div className="animate-fade-in">
                 ...
                 <div className="mt-6 p-4 bg-[#161B26] rounded-lg border border-white/5 text-center text-gray-500 text-sm">
